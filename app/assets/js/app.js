...
$(document).on("click", (e) => {
    if (!$(e.target).hasClass("profile") &&
        $(e.target).parent(".profile").length == 0 &&
        drop
    ) {
        dropdown();
    }
...
function download() {
    $("#update-popup").hide();
    fetch("https://api.github.com/repos/Marius-brt/42-intra-notifier/releases")
        .then((resp) => resp.json())
        .then((dt) => {
            var urlDl = dt[0].assets[0].browser_download_url;
            ipcRenderer.send("open", urlDl);
        });
}

ipcRenderer.on("version", (event, data) => {
    fetch(
            "https://raw.githubusercontent.com/Marius-brt/42-Intra-Notifier/main/version.txt"
        )
